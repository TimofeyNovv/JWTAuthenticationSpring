...
public class AuthenticationController {
    ...
    @Operation(
            ...
            responses = {
                    ...
                    @ApiResponse(responseCode = "400", description = "невалидный формат json или некорректное заполнение полей json", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
            }
    )
    @PostMapping("/login")
    ...
    @Operation(
            ...
            responses = {
                    ...
                    @ApiResponse(responseCode = "409", description = "пользователь с такой почтой уже существует", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                    @ApiResponse(responseCode = "400", description = "невалидный формат json или некорректное заполнение полей json", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
            }
    )
